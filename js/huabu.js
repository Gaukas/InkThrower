...
$(function(){
	...
	var dada = new Image();  
	dada.src = "./img/dd1.jpg";
	var pomo1 = new Image();  
	pomo1.src = "./img/pomo1.png"; 
	if(dada.complete){
		context.drawImage(dada, 0, 0);
	} else {
   		dada.onload = function(){
			context.drawImage(dada, 0, 0);
   		};
   		dada.onerror = function(){
     		window.alert('大大加载失败，请重试');
	   	};
	};
	...
	$('#pomopainter').click(function(event) {
		var pomo_x = Math.floor(Math.random() * 1200);
		var pomo_y = Math.floor(Math.random() * 675);
		if(pomo1.complete){
			context.drawImage(pomo1, pomo_x, pomo_y);
		} else {
			   pomo1.onload = function(){
				context.drawImage(pomo1, pomo_x, pomo_y);
			   };
			   pomo1.onerror = function(){
				 window.alert('墨水加载失败，请重试');
			   };
		};
	});
